...
/**
 * Abstract class for JDBC service. Provides base method to access DB.
 * 
 * @author Lorenzo Natali, GeoSolutions
 *
 */
public abstract class AbstractJDBCService<CONFIGCLASS extends JDBCServiceConfig> {
	...
	/**
	 * initialize a {@link DataSource} form a {@link JDBCServiceConfig} object
	 * 
	 * @param config
	 * @throws IOException
	 */
	public void initializeDSFromConfig(CONFIGCLASS namedConfig)
			throws IOException {
		JDBCServiceConfig config = (JDBCServiceConfig) namedConfig;
		if (config.isJndi()) {
	 ...
	 * @throws SQLException
	 */
	protected PreparedStatement getJDBCStatement(String key, Properties props,
	...
	/**
	 * helper method, if any of the parametres is not null, try to close it and
	 * throw away a possible {@link SQLException}
	 * 
	 * @param con
	 * @param ps
	 * @param rs
	 */
	protected void closeFinally(Connection con, PreparedStatement ps,
			ResultSet rs) {
		try {
			if (rs != null)
				rs.close();
	...
	/**
	 * Get a new connection from the datasource, check/set autocommit == false
	 * and isolation level according to {@link #DEFAULT_ISOLATION_LEVEL}
	 * 
	 * @return
	 * @throws SQLException
	 */
	...
	/**
	 * Invokes {@link #createTables()}, but only if {@code config.isCreatingTables() == true}
	 * and {@link #tablesAlreadyCreated()} returns false.
	 * @param config
	 * @throws IOException
	 */
	public void createTablesIfRequired(CONFIGCLASS config)
			throws IOException {
	...
	/**
	 * create tables and indexes, statement order defined by
	 * {@link #getOrderedNamesForCreate()}
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Checks if the sql statements contained in props can be prepared against
	 * the db
	 * 
	 * @param props
	 * @return return error protocol containing key,statement and
	 *         {@link SQLException}. The key is created as follows:
	 * 
	 *         property key + "|" + statement string
	 * 
	 * @throws IOException
	 */
	protected Map<String, SQLException> checkSQLStatements(Properties props)
			throws IOException {
	 ...
	 * 
	 * If the file does not exist, check for a template file contained in the
	 * jar with the same name, if found, use it.
	 * 
	 ...
	 *            , target location
	 * @param namedRoot
	 *            , parent dir if fileName is relative

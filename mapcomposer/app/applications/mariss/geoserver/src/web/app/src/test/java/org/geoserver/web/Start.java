...
public class Start {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            /*
             * Constraint constraint = new Constraint(); constraint.setName(Constraint.__BASIC_AUTH);; constraint.setRoles(new
             * String[]{"user","admin","moderator"}); constraint.setAuthenticate(true);
             * 
             * ConstraintMapping cm = new ConstraintMapping(); cm.setConstraint(constraint); cm.setPathSpec("/*");
             * 
             * SecurityHandler sh = new SecurityHandler(); sh.setUserRealm(new HashUserRealm("MyRealm","/Users/jdeolive/realm.properties"));
            ...
            log.info("Starting Jetty (To shutdown write \'stop\' and hit enter)");
            jettyServer.start();

            /*
             * Reads from System.in looking for the string "stop\n" in order to gracefully terminate the jetty server and shut down the JVM. This way
             * we can invoke the shutdown hooks while debugging in eclipse. Can't catch CTRL-C to emulate SIGINT as the eclipse console is not
             * propagating that event
             */

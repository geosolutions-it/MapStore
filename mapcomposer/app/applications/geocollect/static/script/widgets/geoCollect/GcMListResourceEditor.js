...
mxp.widgets.GcMListResourceEditor = Ext.extend(Ext.Panel, {
    ...
    initComponent : function () {
        ...
        //Filtro lo store dai campi geometry

        //creo i combo box
        this.listName = new Ext.form.ComboBox(Ext.apply(defaultComboList, {
                    fieldLabel : this.nameFieldLabel,
                    store : this.store
                }));
        this.listDescription = new Ext.form.ComboBox(Ext.apply(defaultComboList, {
                    fieldLabel : this.descriptionFieldLabel,
                    store : this.store
                }));
        this.listOrdering = new Ext.form.ComboBox(Ext.apply(defaultComboList, {
                    fieldLabel : this.orderingFieldLabel,
                    store : this.store
                }));
        this.listIcon = new Ext.form.ComboBox(Ext.apply(defaultComboList, {
                    fieldLabel : this.iconFieldLabel,
                    store : this.store,
                    listeners : {
                ...
                items : [
                    this.listIcon,
                    {
                        ...
                        handler : function (btn) {
                            ...
                            //recuperi ultimo filtro e controlli se valido prima di poter aggiungerne un'altro
...
*/
    ...
    canCommit :function(){
        //Condizioni per sapere se è committabile 
        //Deve avere selezionato tutto e avere almeno un filtro
        if ( this.listName.getValue()
            && this.listDescription.getValue()
            && this.listIcon.getValue()
            && this.listOrdering.getValue()
    ...
    setPriorityValuesColors:function(res){
        ...
        if(res.priorityField == this.listIcon.getValue()){
            ...
            if(filterArray[0]){
                filterArray[0].findParentByType('container').destroy();
            }
    ...
    validatePriority:function(){
        var filterArray = this.filterContainer.findByType('mxp_filterfield');
        if(filterArray.length > 0 || filterArray[0] && filterArray[0].validateValue()){
            return true;
        }
        else {
            return false;

                ...
                load: function() {
                    ...
                    if (!this.dbstore.reader.raw || !this.dbstore.reader.raw.service) {
					 ...
					 var recs = Ext.isArray(arguments[1]) ? arguments[1] : arguments[0];
                    	...
                    	//TODO::Vanno eliminati i layer con postfisso _sop
                       if(recs){
...
this.autoScroll=true;
	...
	if(scope.seg_fieldStore.getCount()>0 && scope.sop_fieldStore.getCount()>0 ){
			if(scope.firstCall) {scope.fireEvent("ready",scope);
		...
		if(this.typeName){
			...
			schema_sop=	{ 
             ...
             var pr=new  OpenLayers.Format.JSON();    
                    ...
                    return pr.write({"schema_seg":schema_seg,"schema_sop":schema_sop},true);
                }},
 ...
 getFieldsObj:function(store){
 	...
 	return recObj;
                ...
                //expect string OpenLayers.Format.JSON
    loadResourceData: function(resource){
    			if(resource instanceof OpenLayers.Format.JSON)res=resource;
    			else{
    				var pr=new  OpenLayers.Format.JSON();
...
this.seg_fieldStore.each(function(r){
	records.push(r.copy().data);
});
return records;
},
getSop_raw:function(){
	var records=[];
this.sop_fieldStore.each(function(r){
	records.push(r.copy().data);
});
return records;
},
...
new_seg=	new Ext.data.JsonStore({
	fields: this.parseFields,
	proxy:new Ext.data.MemoryProxy(this.getSeg_raw())	
})	
...
getSop_Store:function(){
	new_sop= new Ext.data.JsonStore({
	fields:this.parseFields,
	proxy:new Ext.data.MemoryProxy(this.getSop_raw())	
})	

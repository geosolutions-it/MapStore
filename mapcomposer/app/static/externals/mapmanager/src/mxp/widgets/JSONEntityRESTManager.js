...
mxp.widgets.JSONEntityRESTManager = Ext.extend(Ext.Panel, {
     ...
     */
    baseUrl: "",
    /** NOTE:: editors must be serializable **/
    entities:[{
            id: "Crops",
            name:"Crop",
            canCreate:true,
        ...
        /** field to display as id **/
            displayField: "label",
            fields:[{name: "id", mapping: "id"},
                    {name: "label", mapping: "label"},
                    {name: "prod_default_unit", mapping: "prod_default_unit"},
                    {name: "area_default_unit", mapping: "area_default_unit"},
                    {name: "yield_default_unit", mapping: "yield_default_unit"},
                    {name: "rabi",header:"Rabi", mapping: "rabi", type:'boolean'},
                    {name: "kharif",header:"Kharif", mapping: "kharif", type:'boolean'}],
            autoExpandColumn: 'label',
            columns:[ 
                    {name: "id",header:"id", mapping: "id"},
                    {name: "label",header:"Label", mapping: "label"},
                    {name: "prod_default_unit", header: "Default Production Uom", mapping: "prod_default_unit"},
                    {name: "area_default_unit", header: "Default Area Uom", mapping: "area_default_unit"},
                    {name: "yield_default_unit", header: "Default Yield Uom", mapping: "yield_default_unit"},
                    {name: "rabi",  header:"Rabi",   mapping: "rabi", xtype:'booleancolumn', width: 100,maxWidth:100,fixed:true, trueText:'Yes',falseText:''},
                    {name: "kharif",header:"Kharif", mapping: "kharif", xtype:'booleancolumn', width: 100,maxWidth:100,fixed:true, trueText:'Yes',falseText:''}
            ],
            editHeight:270,
            form:{
                edit: [
                    {name: "id",    fieldLabel:"Id",    xtype:"textfield", readOnly:true,allowBlank: false },
                    {name: "label", fieldLabel:"Label", xtype:"textfield",allowBlank: false },
                    {
                        fieldLabel:"Season", 
                        xtype:"checkboxgroup", 
                        name:"seasons",
                        items:[
                            {  boxLabel:"Rabi",    name:"rabi",inputValue:true},
                            {  boxLabel:"Kharif",  name:"kharif",inputValue:true}
                        ]
                    },{
                        ...
                        store:{
                            ...
                            root: 'data',
                            totalProperty: 'total',
                            url: 'http://localhost:8080/opensdi2-web/mvc/cip/uom/filterby?attributename=cls&valueLike=production',
                            idProperty: 'id',
                            fields: [
                                'id',
                                'name'
                        ...
                        store:{
                            ...
                            root: 'data',
                            totalProperty: 'total',
                            url: 'http://localhost:8080/opensdi2-web/mvc/cip/uom/filterby?attributename=cls&valueLike=area',
                            idProperty: 'id',
                            fields: [
                                'id',
                                'name'
                        ...
                        name: "yield_default_unit", fieldLabel: "Yield Uom",allowBlank: false,
                        xtype:'combo', 
                        triggerAction:'all',
                        mode:'local',
                        displayField: 'name',
                        ...
                        store:{
                            ...
                            root: 'data',
                            totalProperty: 'total',
                            url: 'http://localhost:8080/opensdi2-web/mvc/cip/uom/filterby?attributename=cls&valueLike=yield',
                            idProperty: 'id',
                            fields: [
                                'id',
                                'name'
                ...
                create: [
                    {name: "id",    fieldLabel:"Id",    xtype:"textfield", reandonly:false, allowBlank: false },
                    {name: "label", fieldLabel:"Label", xtype:"textfield",allowBlank: false },
                    {
                        fieldLabel:"Season", 
                        xtype:"checkboxgroup", 
                        ...
                        name: "yield_default_unit", fieldLabel: "Production Uom",allowBlank: false,
                        xtype:'combo', 
                        triggerAction:'all',
                        mode:'local',
                        displayField: 'name',
            ...
            api: {
             ...
             /** field to display as id **/
            displayField: "label",
            fields:[{name: "factor", mapping: "factor"},
                    {name: "label", mapping: "label"},
                    {name: "aggregation", mapping: "aggregation"},
                    {name: "unit", mapping: "unit"}
            ],
            autoExpandColumn: 'label',
            
            columns:[ 
                    {name: "factor",header: "Factor", mapping: "factor", allowBlank: false},
                    {name: "label", header: "Label", mapping: "label",allowBlank: false},
                    {name: "aggregation", header: "Aggregation",  width: 100,maxWidth:100,fixed:true, mapping: "aggregation"},
                    {name: "unit", header: "Unit", width: 100,maxWidth:100,fixed:true,  mapping: "unit"}
            ],
            editHeight:200,    
            form: { 
                edit:[
                        {name: "factor",    fieldLabel:"Factor",    xtype:"textfield", readOnly:true,allowBlank: false },
                        {name: "label", fieldLabel:"Label", xtype:"textfield",allowBlank: false },
                        {name: "unit", fieldLabel:"Unit", xtype:"textfield" },
                        {fieldLabel:"Aggregation", xtype:"radiogroup", 
                         items:[
                            {  boxLabel:"avg",name:"aggregation", inputValue:"avg"},
                            {  boxLabel:"sum",name:"aggregation",   inputValue:"sum"}
                        ]}
                ],
                create:[
                        {name: "factor",    fieldLabel:"Factor",    xtype:"textfield",allowBlank: false },
                        {name: "label", fieldLabel:"Label", xtype:"textfield" },
                        {name: "unit", fieldLabel:"Unit", xtype:"textfield" },
                        {fieldLabel:"Aggregation", xtype:"radiogroup", 
                         items:[
                            {  boxLabel:"avg", inputValue:"avg",name:"aggregation",checked:true},
                            {  boxLabel:"sum",   inputValue:"sum",name:"aggregation"}
                        ]}
                ]
            }
        },{
            ...
            fields:[
                ...
                {name: "coefficient", mapping: "coefficient"},
                {name: "filter", mapping: "filter"}

            ],
            columns:[ 
                {name: "id",header: "ID", width: 100,maxWidth:100,fixed:true, mapping: "id"},
                {name: "name", header: "Label", mapping: "name"},
                {name: "shortname",header: "Short Name", width: 100,maxWidth:100,fixed:true, mapping: "shortname"},
                {name: "description", mapping: "description",header: "Description"},
                {name: "cls", header: "Class", width: 100,maxWidth:100,fixed:true, mapping: "cls"},
                {name: "coefficient", header: "factor", width: 100,maxWidth:100,fixed:true, mapping: "coefficient"},
                {name: "filter", header:"filter",  width: 100,maxWidth:100,fixed:true, mapping: "filter"}
            
            ],
            editHeight:310,    
            form: { 
                edit:[
                        {name: "id",fieldLabel: "ID", readOnly:true,allowBlank: false},
                        {name: "name", fieldLabel: "Label",allowBlank: false},
                        {name: "shortname",fieldLabel: "Short Name", mapping: "shortname",allowBlank: false},
                        {name: "description", xtype:'textarea', fieldLabel: "description",header: "Description"},
                        {name: "cls", fieldLabel: "Class",allowBlank: false,
                            xtype:'combo', 
                            triggerAction:'all',
                            mode:'local',
                            displayField: 'label',
                        ...
                        {name: "coefficient", fieldLabel: "factor", mapping: "coefficient",xtype:"numberfield",decimalPrecision:10,allowBlank: false},
                        {name: "filter", fieldLabel:"filter", mapping: "filter"}
                ],
                create:[
                        {name: "id",fieldLabel: "ID", mapping: "id",allowBlank: false},
                        {name: "name", fieldLabel: "Label", mapping: "name",allowBlank: false},
                        {name: "shortname",fieldLabel: "Short Name", mapping: "shortname",allowBlank: false},
                        {name: "description", xtype:'textarea', fieldLabel: "description",header: "Description"},
                        {name: "cls", fieldLabel: "Class",allowBlank: false,
                            xtype:'combo',

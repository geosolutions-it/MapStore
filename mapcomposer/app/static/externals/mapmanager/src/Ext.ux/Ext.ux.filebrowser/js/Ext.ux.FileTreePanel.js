	...
	,initComponent:function() {
		...
		if(!this.loader) {
			this.loader = new Ext.tree.TreeLoader({
				 url:this.url
				,baseParams:{action:'get_folderlist'}
				,listeners:{
					beforeload:{scope:this, fn:function(loader, node) {
						loader.baseParams.folder = this.getPath(node);
					}}
				}
			});
		}
	        ...
	        if(true === o.success) {
	            switch(options.params.action) {
	                ...
	                case 'file_rename':
		                ...
		                if(true !== this.eventsSuspended) {
		                    this.fireEvent('rename', this, options.node, options.params.name, options.params.oldName);
		                }
                    break;
	            }
			...
			// process command failure
				...
				switch(options.params.action) {
					...
					case 'file_rename':
						...
						if(true !== this.eventsSuspended) {
							this.fireEvent('renamefailure', this, options.node, options.params.name, options.params.oldName);
						}
					break;

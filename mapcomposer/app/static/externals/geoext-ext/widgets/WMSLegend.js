...
GeoExt.WMSLegend = Ext.extend(GeoExt.LayerLegend, {
    ...
    getLegendUrl: function(layerName, layerNames) {
		...
		if(lowerUrl.indexOf("request=getlegendgraphic") != -1){
			...
			var params = Ext.applyIf({}, this.baseParams);
			
			//
			// SERVICE check
			//		
			var service = lowerUrl.indexOf("service=wms");
			if(service == -1){
				url = Ext.urlAppend(url, "SERVICE=WMS");
			}
		
			//
			// FORMAT check
			//
			var format = lowerUrl.indexOf("format=");
			if(format != -1 && params.FORMAT){
				url = url.replace(/format=([a-z0-9\/\%]+)/i, "format=" + params.FORMAT);
			...
			// HEIGHT check
			//
			var height = lowerUrl.indexOf("height=");
			if(height != -1 && params.HEIGHT){
				url = url.replace(/height=([0-9]+)/i, "height=" + params.HEIGHT);
				delete params.HEIGHT;
			}else if(height == -1){
				url = Ext.urlAppend(url, "HEIGHT=20");
			}	
			
			//
			// WIDTH check
			//
			var width = lowerUrl.indexOf("width=");
			if(width != -1 && params.WIDTH){
				url = url.replace(/width=([0-9]+)/i, "width=" + params.WIDTH);
				delete params.WIDTH;
			}else if(width == -1){
				url = Ext.urlAppend(url, "WIDTH=20");
			}	

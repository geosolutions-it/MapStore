...
GeoExt.data.PrintProvider = Ext.extend(Ext.util.Observable, {
    ...
    constructor: function(config) {
        ...
        this.fullLayouts = new Ext.data.JsonStore({
            root: "layouts",
            fields: [
                "name",
                {name: "size", mapping: "map"},
    ...
    encoders: {
        "layers": {
            "WMS": function(layer) {
				...
				var printParams = this.initialConfig.printParams;
				if(printParams){
					for(var printParam in printParams){
						enc.customParams[printParam] = printParams[printParam];
					}
				}

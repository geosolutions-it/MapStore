    ...
    */
    addLayer: function(evt) {
		if (evt.layer) {
			...
			//
			// Added because If the layer is removed before the loadend, 
			// the 'decreaseCounter' is not called
			//
			evt.layer.events.register('removed', this, function(e){
				if(e.layer.loading){
    ...
    setMap: function(map) {
		...
		for (var i = 0; i < this.map.layers.length; i++) {
			...
			// the 'decreaseCounter' is not called
			//
			layer.events.register('removed', this, function(e){
				if(e.layer.loading){
					this.decreaseCounter();
				}
			});
		...
		if (this.map) {
			...
			if (this.map.layers) {
				for (var i = 0; i < this.map.layers.length; i++) {
					...
					// the 'decreaseCounter' is not called
					//
					layer.events.unregister('removed', this, function(e){
						if(e.layer.loading){
							this.decreaseCounter();
						}
					});

...
OpenLayers.WPSClient = OpenLayers.Class({
    ...
    initialize: function(options) {
        ...
        this.events = new OpenLayers.EventsExt(this);
        this.servers = {};
    ...
    describeProcess: function(serverID, processID, callback, scope) {
        ...
        if (!server.processDescription[processID]) {
            ...
            //    var pattern=/(.+:\/\/)?([^\/]+)(\/.*)*/i; 
               /* var params= "?SERVICE=WPS&VERSION="+server.version
                    +"&REQUEST=DescribeProcess"
                    +"&IDENTIFIER="+processID;
                var mHost; 
                var url;
                mHost=pattern.exec(server.url); 
                url = mHost[2] == location.host ? server.url+params :
                OpenLayers.ProxyHost + encodeURIComponent(server.url+params);

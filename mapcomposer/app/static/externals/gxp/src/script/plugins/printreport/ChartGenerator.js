...
/** api: constructor
 *  .. class:: AgrometChartGenerator(config)
 *
 *    Component to generate the agromet data charts for the PrintReportHelper
 *
 */
gxp.plugins.printreport.ChartGenerator = Ext.extend(gxp.plugins.printreport.Generator, {
    ...
    generateChart:function(store,listVar, allPakistanRegions){
        this.chartsSVG = [];

        this.chartPanel = Ext.apply(this.chartPanelConfig, {
            url: this.url,
            target:this.target,
            form: this.form,
            listeners:{
    ...
    generate:function(store,listVar, allPakistanRegions){
        this.chartsSVG = [];
        this.generateChart();
    },
    ...
    getChartPanel: function(){
        return this.reportWindow.items.get(this.reportWindow.items.getKeys()[0]);
    },

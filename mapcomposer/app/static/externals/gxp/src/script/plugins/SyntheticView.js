    ...
    addRisk: function(layers, bounds, status) {                
        ...
        if(status.formulaInfo.dependsOnTarget) {
            if(this.isSingleTarget()) {
                ...
                this.addFormula(layers, bounds, status, 98, this.formulaRiskLayer[resolution], formulaDescHum, formulaUdmSoc, env, roi, true);
            } else if(this.isAllNotHumanTargets()) {
                this.addFormula(layers, bounds, status, 99, this.formulaRiskLayer[resolution], formulaDescNotHum, formulaUdmEnv, env, roi, true);
            } else {
                this.addFormula(layers, bounds, status, 98, this.formulaRiskLayer[resolution], formulaDescHum, formulaUdmSoc, envhum, roi, false);
                this.addFormula(layers, bounds, status, 99, this.formulaRiskLayer[resolution], formulaDescNotHum, formulaUdmEnv, envamb, roi, false);                    
                this.addFormula(layers, bounds, status, 100, this.mixedFormulaRiskLayer[resolution], formulaDescTot, formulaUdm, mixedenv, roi, true);
            }
        } else {

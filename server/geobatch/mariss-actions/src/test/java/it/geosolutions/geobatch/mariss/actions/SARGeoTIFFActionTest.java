...
public class SARGeoTIFFActionTest {
    private static final List<String> testNames = Arrays.asList(
            "TDX1_SAR__MGD_RE___SM_S_SRA_20110901T175713_20110901T175721_DER.zip",
    ...
    private static final String transforms = "{" + " \"transforms\":[" + "{"
            + "  \"type\":\"GdalWarpTransform\", " + "  \"options\":[ " + "    \"-t_srs\" ,"
            + "    \"EPSG:4326\" " + "]"
            /*
             * }, { "type":"GdalTranslateTransform", "options":[ "-co", "TILED=YES", "-co", "BLOCKXSIZE=512", "-co", "BLOCKYSIZE=512" ] }, {
             * "type":"GdalAddoTransform", "options":[ "-r", "average" ], "levels":[2, 4, 8, 16]
             */
            + " }" + " ]" + "  }";
    ...
    public void testExecutionSteps() throws Exception {
        ...
        /*
         * Queue<EventObject> q = new PriorityQueue<EventObject>(); q.add(event); action.execute(q);
         */
        ...
        try {
            ...
            DataStore ds = FeatureConfigurationUtil
                    .createDataStore(action.configuration.getOutputFeature());
            assertTrue(ds != null);
    ...
    private SARGeoTiffActionConfiguration createDummyConfig() {
        ...
        // String tranformOpts ="{\"type\": \"GdalWarpTransform\",\"options\": [ \"-t_srs\", \"EPSG:4326\"]},{\"type\": \"GdalTranslateTransform\",
        // \"options\": [ \"-co\", \"TILED=YES\", \"-co\", \"BLOCKXSIZE=512\", \"-co\", \"BLOCKYSIZE=512\"]},{\"type\":
        // \"GdalAddoTransform\",\"options\": [ \"-r\", \"average\"],\"levels\" : [2, 4, 8, 16]}";
        ...
        SARGeoTiffActionConfiguration config = new SARGeoTiffActionConfiguration("Test", "name",
                "test configuration");

...
public class ActionWrapper extends BaseAction<EventObject> {
    public ActionWrapper(ActionWrapperConfiguration actionConfiguration)
    ...
    private void initialize() throws InterruptedException, IllegalArgumentException, IOException {
        ...
        for (ActionConfiguration actionConfig : configuration.getActions()) {
            ...
            final GenericActionService service = catalog.getResource(actionServiceID,
                    GenericActionService.class);
            ...
            if (service != null) {
                ...
                if (configuration.isIgnoreConfigurationFail()
                        || service.checkConfiguration(actionConfig)) {
                    ...
                    for (ProgressListenerConfiguration plConfig : actionConfig
                        ...
                        if (progressListenerService != null) {
                            ...
                            throw new IllegalArgumentException("Could not find '"
                                    + listenerServiceID + "' listener," + " declared in "
                                    + actionConfig.getId() + " action configuration," + " in "
    ...
    public Queue<EventObject> execute(Queue<EventObject> events) throws ActionException {
        ...
        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace(
                    "Applying " + actions.size() + " actions on " + events.size() + " events.");
        }
        ...
        try {
            for (BaseAction<EventObject> action : this.actions) {
                try {
                    float progress = 100f * step / this.actions.size();
                    listenerForwarder.setProgress(progress);// TODO progress is global
                    listenerForwarder.setTask("Running " + action.getName() + "(" + (step + 1) + "/"
                            + this.actions.size() + ")");
                    ...
                    // execute the action
                    Queue<EventObject> inputEvents = getInputEvents(action, events, outputs,
                            originalEvents, preprocessedEvents);
                    ...
                    if (events == null) {
                        throw new IllegalArgumentException(
                                "Action " + action.getName() + " returns a null queue.");
                    }
                    if (events.isEmpty()) {
                        if (LOGGER.isWarnEnabled()) {
                            LOGGER.warn("Action " + action.getName() + " left no event in queue.");
                        }
                    }
                    step++;
                    ...
                    if (LOGGER.isErrorEnabled()) {
                        LOGGER.error(
                                "Action threw an unhandled exception: " + e.getLocalizedMessage(),
                                e);
                    }
                    ...
                    if (!currentAction.isFailIgnored()) {
                        if (events == null) {
                            throw new IllegalArgumentException(
                                    "Action " + action.getName() + " left no event in queue.");
                        } else {
                            events.clear();
                        }
        ...
        if (events != null && !events.isEmpty()) {
            LOGGER.info(
                    "There are " + events.size() + " events left in the queue after last action ("
     ...
     * @return
     */
    protected Queue<EventObject> generateOutput(Queue<EventObject> originalEvents,
            Map<String, Queue<EventObject>> outputs) {
        if (configuration.getOutputId() != null
                && outputs.containsKey(configuration.getOutputId())) {
    ...
    /**
     * Find input source and dispatch to the action execution. Allows to get ORIGINAL_EVENTS, PREPROCESSED_EVENTS to get input events at all You can
     * dispatch also multiple inputs/outputs separating action ids with a ","
     * 
     * E.g. ORIGINAL_EVENTS,csv will get the original events (of the wrapper) and the output of the action named csv as input of the action.
     * 
     ...
     * @return
     */
    private Queue<EventObject> getInputEvents(BaseAction<EventObject> action,
            Queue<EventObject> events, Map<String, Queue<EventObject>> outputs,
            Queue<EventObject> originalEvents, Queue<EventObject> preprocessedEvents) {
     ...
     */
    private Queue<EventObject> collectEvents(Map<String, Queue<EventObject>> outputs,
            Queue<EventObject> originalEvents, Queue<EventObject> preprocessedEvents, String inIds,
            ActionOption opt) {
    ...
    protected void setupAction(BaseAction action, int step) throws IllegalStateException {
        ...
        if (!actionTempDir.mkdirs()) {
            throw new IllegalStateException(
                    "Unable to create the action temporary dir: " + actionTempDir);
        }

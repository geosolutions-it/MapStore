...
/**
 * Action that dispatch to many flows the files to parse. gets a <FileMetadataWrapper> object and a <FileSystemEvent> object with a directory as
 * source It has configured a filename, the <FileMetadataWriter> will be written into the directory and writes a XStream Returns
 * <FileSystemEvent> with the file created
 * 
    ...
    public Queue<EventObject> execute(Queue<EventObject> events) throws ActionException {
        ...
        while (events.size() > 0) {
            ...
            if (LOGGER.isTraceEnabled()) {
                LOGGER.trace("Working on incoming event: " + event.getSource());
            }

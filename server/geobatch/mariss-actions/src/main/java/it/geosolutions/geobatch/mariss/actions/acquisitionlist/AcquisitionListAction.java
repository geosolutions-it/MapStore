...
public class AcquisitionListAction extends RemoteServiceHandlingAction {
	...
	public AcquisitionListAction(
			AcquisitionListActionConfiguration configuration)
			throws IOException {
		super(configuration);
	...
	protected Collection<? extends EventObject> ingestServiceData(
			...
			// input dir
			 DataStore dataStore, int usersSize, int userIndex,
			int serviceSize, int serviceIndex) {
		...
		try {
			...
			if ("AOI".equals(service.getStatus())) {
				...
				final String serviceIDFtpPath = remotePath + FTP_SEPARATOR
						+ user + FTP_SEPARATOR + service.getServiceId();
				...
				for (String folder : serviceFolders) {
					...
					String remoteRelativeFolder = user + FTP_SEPARATOR
							+ service.getServiceId() + FTP_SEPARATOR + folder;
					...
					if (isObservableServiceFolder(service, folder)) {
						...
						final String currentRemoteFolder = remotePath
								+ FTP_SEPARATOR + remoteRelativeFolder;

						if (LOGGER.isInfoEnabled()) {
							LOGGER.info("Get files in the folder "
									+ currentRemoteFolder);
						}
						...
						for (int serviceFolderFileIndex = 0;  serviceFolderFileIndex< serviceFolderFilesSize;serviceFolderFileIndex++) {
							...
							File inputDir = new File(getTempDir(),user + LOCAL_SEPARATOR + service.getServiceId() + LOCAL_SEPARATOR + ACQ_LIST_FOLDER);
							final String csvFileName = CSVIngestUtils
									.getUserServiceFileName(
											fileName,
											user,
						...
						if (subconfigurations != null
							...
							for (String key : subconfigurations.keySet()) {
								...
								if (action != null) {
									File actionTempDir = new File(getTempDir(),
											key);
									try {
						...
						resultList.addAll(getPostProcessEvents(
								pendingProductFiles, dataStore, user, service,
								localRelativeFolder, serverResultProtocol,
								folder, folder, folder, resultTimeout,
								resultConnectMode, resultTimeout, usersSize,
								userIndex, serviceSize, serviceIndex,

...
public class GenericFeatureDaoImpl implements GenericDAO<SimpleFeature, Long> {
     ...
     * @throws IOException
     */
    protected Filter getSearchByPkFilter(String[] names, Serializable... pkObjects)
        ...
        if (updateFilter != null) {
            ...
            try {
                ...
                if (source instanceof SimpleFeatureStore) {
                    ...
                    featureStore.setTransaction(transaction);
                    try {
                        featureStore.modifyFeatures(names, attributeValuesList.toArray(),
                                updateFilter);
                        transaction.commit();
                    } catch (Exception problem) {
                        LOGGER.error("Error on commit", problem);
    ...
    public void prepare() {
        try {
            ...
            this.source = ((FeatureStore<SimpleFeatureType, SimpleFeature>) dataStore

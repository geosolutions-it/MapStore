...
public class DataPackageIngestionProcessor extends ProductIngestionProcessor {
    ...
    public DataPackageIngestionProcessor(final DataPackageIngestionConfiguration configuration)
            throws IOException {
        super(configuration);
    ...
    public DataPackageIngestionProcessor(DataStore dataStore, String typeName, int projection) {
    ...
    public DataPackageIngestionProcessor(DataStore dataStore, String typeName, String userName,
            String serviceName) {
    ...
    public boolean canProcess(String filePath) {
        if (filePath.endsWith(".zip") || filePath.endsWith(".tgz")) {
            return true;
        } else {
            return false;
        }
        ...
        try {
            ...
            for (JAXBElement<?> element : shipData.getRest()) {
                ...
                if (LOGGER.isTraceEnabled()) {
                    LOGGER.trace("Reading ship element: " + element.getName() + "=" + value);
                }
            ...
            if (externalId != null) {
                ...
                feature.setAttribute("service_name", userName + "@" + serviceName);
                // other feature elements
                feature.setAttribute("time",
                        timestamp != null ? new Timestamp(timestamp.getMillisecond()) : null);
    ...
    private Queue<EventObject> doProcess(JDBCDataStore dataStore, File file,
                ...
                if ((ev = events.remove()) != null) {
                    ...
                    if (ev instanceof FileSystemEvent) {
                        ...
                        try {
                            ...
                            Queue<EventObject> resultEvents = doProcess(dataStore, file,
                                    (FileSystemEvent) ev);
    ...
    public Collection<? extends EventObject> processCompressedFile(String compressedFile)
        ...
        String zipPath = workingDir
                + compressedFile.substring(compressedFile.lastIndexOf(File.separator));
        ...
        if (zipFolder.exists() && zipFolder.isDirectory()) {
            ...
            for (String fileName : zipFolder.list()) {
                ...
                } else if (fileName.endsWith(".tif") || fileName.endsWith(".nc")) {
    ...
    public List<String> processDataPackage(File file) {
        ...
        try {
            DataPackageType dataPackage = JAXB.unmarshal(file, DataPackageType.class);

            for (DetectedShipReferenceType detectedReferenceShip : dataPackage.getDetectedShips()
                    .getDetectedShipReference()) {

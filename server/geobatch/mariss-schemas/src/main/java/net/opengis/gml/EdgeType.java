...
/**
 * There is precisely one positively directed and one negatively directed node in the boundary of every edge. The negatively and positively directed
 * nodes correspond to the start and end nodes respectively. The optional coboundary of an edge is a circular sequence of directed faces which are
 * incident on this edge in document order. Faces which use a particular boundary edge in its positive orientation appear with positive orientation on
 * the coboundary of the same edge. In the 2D case, the orientation of the face on the left of the edge is "+"; the orientation of the face on the
 * right on its right is "-". An edge may optionally be realised by a 1-dimensional (curve) geometric primitive.
 * 
 * <p>
...
public class EdgeType extends AbstractTopoPrimitiveType {
     ...
     * present inside the JAXB object. This is why there is not a <CODE>set</CODE> method for the directedFace property.
     * 
     * <p>
     ...
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list {@link DirectedFacePropertyType }
     * 
     * 
     */
     ...
     * <p>
     * This accessor method returns a reference to the live list, not a snapshot. Therefore any modification you make to the returned list will be
     * present inside the JAXB object. This is why there is not a <CODE>set</CODE> method for the directedNode property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * 
     ...
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list {@link DirectedNodePropertyType }
     * 
     * 
     */
